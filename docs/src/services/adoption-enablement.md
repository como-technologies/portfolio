...
**Artifact out.** Decisions in daily practice: adopted workflows, changed
defaults, merged changes, and teams that keep going after Como leaves. The
deliberately unglamorous deliverable — and the one programs usually miss.

**Human gates.** All of it. Enablement *is* the human gate: what to roll out
first, what to defer, when a decision needs to go back to Prescribe because
reality disagreed with it. Where conduit enters via the agentic-delivery
pilot, its three named gates apply on top.

**Measure hooks.** Enablement claims are checked, not asserted:
[Measurement setup](./measurement-setup.md) captures where capacity actually
goes (tuesday) and how the team actually feels about the change (pulse), so
the rollout's effect is visible in the next iteration's evidence rather than
in a status slide.
