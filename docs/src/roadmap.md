...
- **Shipped** — the authoring kit (instructions, Como skills, reference harness configs, starter content, and a captured worked example, all in the llm-wiki repo), and the guarded MCP write path for decisions (`adroit mcp --allow-write`, its ADR-0021) so MCP-only harnesses author them too. The harness-first loop runs today with Como driving and SMEs alongside; the climb to a client team running it without Como in the room is a rung the offering earns on its own evidence.
