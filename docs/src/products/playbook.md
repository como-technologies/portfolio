...
a team can maintain is not prose that silently rots, but a small, honest,
typed record of what was decided, why, and what it cost — validated
mechanically, so rot fails a gate instead of accumulating. It is the
[knowledge base](../kb-spec.md)'s decision-and-guidance content, packaged as
a product a team can clone.
...
- **An adroit-managed ADR corpus** with an ADR template
...
- **The book**: everything publishes as a self-hosted mdBook static site,
  so the content lives where the team already operates.
...
chapter describes how it sits in the loop — including the honest note that
the published template still pins adroit at v0.2.0, pre-KB-cutover; the
seed-into-a-space gate reaches it in a coming wave.

## The delivered proof point

The pattern has shipped once as a client engagement: a playbook built for
an enterprise cloud and Kubernetes platform team — twenty-one ADRs across
the lifecycle statuses plus six guides, delivered as a self-hosted static
site (mdBook + Docker + nginx) running where the team operates. It predates
the generic template and is the engagement the template's structure and
process patterns were distilled from — content written fresh, with a
CI-enforced rule that no client material enters the product.
